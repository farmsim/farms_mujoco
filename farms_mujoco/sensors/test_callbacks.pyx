...
cpdef mjcb_sensor(mj_model, mj_data, stage):
    """ mujoco sensor callback """

    cy_mjcb_sensor(mj_model, mj_data, stage)
...
cdef void cy_mjcb_sensor(object mj_model, object mj_data, int stage):
    """ Cython function for faster sensor updates """

    # mujoco enums
    # cdef object mj_enums = mjbindings.enums

    # # number of user sensors
    # cdef int nu_sensors = mj_model.nuser_sensor
    # cdef int n_sensors = mj_model.nsensor
    # # user sensor ids
    # user_sensors_id = np.nonzero(
    #         mj_model.sensor_type == mj_enums.mjtSensor.mjSENS_USER
    # )[0]
